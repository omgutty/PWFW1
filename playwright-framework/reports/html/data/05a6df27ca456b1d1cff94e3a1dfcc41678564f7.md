...
- Name: ui\login.spec.ts >> Login test
...
```ts
  ...
  3  | 
  4  | test('Login test', async ({ page, loginPage }) => {
  5  |   await page.goto('/');
  6  | 
  ...
  13 |   //specifically failing to check the video recording 
> 14 |   await expect(page).toHaveURL(/wrong-url/);
     |                      ^ Error: expect(page).toHaveURL(expected) failed
  15 |   
  16 | });
```